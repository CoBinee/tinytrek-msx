// g++ -o image2sprite `sdl2-config --cflags --libs` -lSDL2_image image2sprite.cpp
    ...
    // ファイルへの出力
    {
        ...
        for (int c_y = 0; c_y < c_height; c_y++) {
            for (int c_x = 0; c_x < c_width; c_x++) {
                for (int s_x = 0; s_x < 2; s_x++) {
                    for (int s_y = 0; s_y < 2; s_y++) {
                        fprintf(outfile, "    .db     ");
                        for (int p_y = 0; p_y < 8; p_y++) {
                            unsigned char bits = 0x00;
                            for (int p_x = 0; p_x < 8; p_x++) {
                                int x = c_x * 16 + s_x * 8 + p_x;
                                int y = c_y * 16 + s_y * 8 + p_y;
...
{
    // ウィンドウの作成
    SDL_Window *window = SDL_CreateWindow("image2sprite", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, image->w, image->h, SDL_WINDOW_SHOWN);
    if (window == NULL) {
        fprintf(stderr, "error: %s\n", SDL_GetError());
        return -1;
    }
